package com.example.skeleton.common.batch.reader.options;
...
public class QuerydslNoOffsetNumberOptions<T, N extends Number & Comparable<?>>
		extends QuerydslNoOffsetOptions<T> {
	...
	public QuerydslNoOffsetNumberOptions(
			@Nonnull NumberPath<N> field, @Nonnull Expression expression) {
	...
	protected void initFirstId(JPAQuery<T> query) {
		...
		if (isGroupByQuery) {
			currentId =
					clone.select(field).orderBy(expression.isAsc() ? field.asc() : field.desc()).fetchFirst();
		} else {
			currentId = clone.select(expression.isAsc() ? field.min() : field.max()).fetchFirst();
		}
	}
	...
	protected void initLastId(JPAQuery<T> query) {
		...
		if (isGroupByQuery) {
			lastId =
					clone.select(field).orderBy(expression.isAsc() ? field.desc() : field.asc()).fetchFirst();
		} else {
	...
	public JPAQuery<T> createQuery(JPAQuery<T> query, int page) {
		...
		return query.where(whereExpression(page)).orderBy(orderExpression());
	}

	private BooleanExpression whereExpression(int page) {
		return expression
				.where(field, page, currentId)
